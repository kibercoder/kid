...
function getClassLinewidth(classes, string){
...
function setZindex(idwrapper){
	var wrapper = document.getElementById(idwrapper);
	var elem = wrapper.getElementsByTagName('canvas');
	for (var i = 0; i < elem.length; i++) {
		elem[i].zIndex = 11+i;
		var canvasElement = elem[i].zIndex;
		alert(canvasElement);
	}
}

var zIndexLocationDefault = 0;
var setOverLocation = '';
var zIndexLocation = Array();

function setAreaOver(obj,id,bd,co,op,nb,f,z) {

	areaIdString = obj.getAttribute("id").substring(5);
	var location = document.getElementById(areaIdString);
	//var location = document.getElementsByClassName(areaIdString)[0];

	//console.log('id = ' + id + ', bd = ' + bd + ', co = ' + co + ', op = ' + op +', nb = ' + nb + ', f = ' + f + ', z = ' + z);
	//map_of_kidwork_canvas

	if ((z == 0 && areaIdString == 'hospital') 
	|| (z == 1 && areaIdString == 'construction')
	|| (z == 2 && areaIdString == 'filmstudio')
	|| (z == 3 && areaIdString == 'radiostation')
	|| (z == 4 && areaIdString == 'redaction')
	|| (z == 5 && areaIdString == 'studio')
	|| (z == 6 && areaIdString == 'stylist')
	|| (z == 7 && areaIdString == 'restaurant')
	|| (z == 8 && areaIdString == 'businescenter')
	|| (z == 9 && areaIdString == 'tv-tower')
	|| (z == 10 && areaIdString == 'academy')
	|| (z == 11 && areaIdString == 'bank')
	|| (z == 12 && areaIdString == 'kreml')
	|| (z == 13 && areaIdString == 'carousel')
	|| (z == 14 && areaIdString == 'fountain')
	|| (z == 15 && areaIdString == 'kiosk')
	|| (z == 16 && areaIdString == 'policestation')
	|| (z == 17 && areaIdString == 'whitehouse')
	|| (z == 18 && areaIdString == 'store')){
		
		if (location.style != null && setOverLocation != areaIdString) {

			setOverLocation = areaIdString;

			//$('#tochdevice').text(setOverLocation);

	//zIndexLocationDefault = location.style.zIndex;
	//console.log('id = ' + id + ', bd = ' + bd + ', co = ' + co + ', op = ' + op +', nb = ' + nb + ', f = ' + f + ', z = ' + z);

			//location.style.zIndex = 100;

			//$('.wrapmap canvas').css('zIndex', '110');
			//$('.'+areaIdString+' .wrapmap canvas').css('zIndex', '0');

			//$('.wrapmap').css('display', 'none');
			//$('.'+areaIdString+' .wrapmap').css('display', 'block');
			//$('.wrapmap').css('display', 'block');
			

			//if (!isNaN(zIndexLocation[areaIdString])) {
				//console.log(zIndexLocation[areaIdString]);
	...
	//var nameClass = document.getElementById(id).className;
	//alert(document.getElementById(id).parentNode.getAttribute('class'));


	var a, i, j, d, c, o, b, n, l, r, v, u, x, y, p, k = 0, t = '', r = obj.getAttribute('rel'), context, canvas = document.getElementById(id);

	if(r!=null) {
		d = r.split(","); v = d.unshift(obj.id); 
	}else {
		d = new Array(obj.id); 
	}
	...
	if(isVM) {
		...
		if(f<1) {
			canvas.fading = 0; canvas.style.opacity = 0;
		}
		...
		if(f<1) {
			canvas.fading = 1; fadeCanvas(id,0);
		}
...
function setAreaOut(obj,id,f,z) {
	...
	//var location = document.getElementsByClassName(areaIdString)[0];
	//location.setAttribute('id', areaIdString);


	//console.log(location.style);
	
	// && areaIdString != setOverLocation
	if (location.style != null) {
		//location.style.zIndex = '';

		//$('.wrapmap').css('display', 'block');
		//console.log('setAreaOut');
		setOverLocation = '';
		
		/*if (!isNaN(zIndexLocation[areaIdString])) {
			location.style.zIndex = zIndexLocation[areaIdString];
		}*/
...
function addMapper() {
				...
				if(mapid[0].areas[j].shape.match(/(rect|poly|circle)/i)) {
					if(window.opera||mapid[0].areas[j].coords!='') {
						...
						if(isVM||isIE) {
							...
							//var areaId = mapid[0].areas[j].getAttribute("id");
							//console.log(areaId);
						    ...
						    func = mapid[0].areas[j].getAttribute("onclick"); 
	mapid[0].areas[j].setAttribute("onclick","setAreaOver(this,'"+canvas.id+"','"+b+"','"+c+"','"+o+"',"+n+","+f+","+i+");"+func);

						    //func = mapid[0].areas[j].getAttribute("ontouchstart");
	//mapid[0].areas[j].setAttribute("ontouchstart","setAreaOver(this,'"+canvas.id+"','"+b+"','"+c+"','"+o+"',"+n+","+f+","+i+");"+func); 
							//func = mapid[0].areas[j].getAttribute("ontouchend"); 
	//mapid[0].areas[j].setAttribute("ontouchend","setAreaOut(this,'"+canvas.id+"',"+f+","+i+");"+func);
	...
	mapid[0].areas[j].setAttribute("onmouseout","setAreaOut(this,'"+canvas.id+"',"+f+","+i+");"+func);
		...
		canvas.fading = 0;
		//canvas.setAttribute('data-tooltip', 'value');
